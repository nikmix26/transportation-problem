...
# Подавляем предупреждения, чтобы не засорять консоль
warnings.filterwarnings("ignore")

# Попытка импортировать pulp, если есть
try:
    import pulp
    PULP_AVAILABLE = True
except Exception:
...
def normalize_names(costs_df):
    ...
    Если в costs_df уже есть осмысленные имена - используем их, иначе генерируем S1.. и D1..
    """
    costs = costs_df.copy()
    # suppliers (строки)
...
def balance_problem(costs_df, supply, demand, dummy_cost=0.0):
    """
    Балансировка: если суммарный supply != demand, добавляем фиктивный S0 или D0 с cost=dummy_cost.
    ...
    # Ensure supply/demand lengths match table dimensions. If lengths mismatch, try to adjust by filling zeros.
    ...
    if s_tot > d_tot:
        ...
        # add dummy supplier row named S0
        diff = d_tot - s_tot
        supply.append(diff)
...
def vogel_initial_solution(costs_df, supply, demand):
    ...
    Возвращает матрицу allocation (DataFrame) того же размера, что costs_df.
    """
    ...
    while True:
        ...
        if max_row_pen >= max_col_pen:
            ...
            # pick cheapest available column in row i
...
def solve_with_pulp(costs_df, supply, demand):
    """Формулируем и решаем LP через pulp (если доступен)."""
    ...
    if uploaded is not None:
        try:
            if uploaded.name.lower().endswith('.csv'):
                ...
                st.dataframe(df)
                costs = df
                # попытка выделить supply/demand
            ...
            # --- Чтение листа с матрицей стоимостей ---
            ...
            # --- Чтение supply ---
                ...
                if 'demand' in xls.sheet_names:
                    ...
                    print(d_sheet)
                    try:
                        # стандарт: 2 колонки — D1 / значение
...
# Потребители — таблица
# Нумерация потребителей в списке начинается с 1; фиктивный потребитель (если появится) будет D0 и отобразится с 0.
...
if added is not None:
    ...
    # Отображаем сбалансированные supply/demand как таблицы
    # Для имен: если есть S0/D0, они уже присутствуют в balanced_costs.index/columns
    ...
    # нумерация: поставщики начинаются с 1; фиктивный — S0 (если есть) оставляем его именованным S0
    b_supply_df.index = range(1, len(b_supply_df)+1)
    b_demand_df = pd.DataFrame({"Потребитель": b_demand_labels, "Спрос": balanced_demand})
    # по требованию: в списке спроса все начинаются с единицы, но фиктивный выводится как D0 (он будет присутствовать в b_demand_df как 'D0')
    ...
    # всё уже показано выше

# ---------- Initial solution (Vogel) ----------
...
# Если оптимизация уже была выполнена ранее в сессии, показываем её
...
# Формируем Excel по состоянию: если оптимизация была выполнена — 5 листов, иначе 4 листа.
def make_excel_bytes():
    out = io.BytesIO()
    with pd.ExcelWriter(out, engine='openpyxl') as writer:
        # 1) матрица стоимостей (исходная, до балансировки) — используем costs
        ...
        # 4) решение Вёгеля и стоимость
        alloc_vogel.to_excel(writer, sheet_name="Решение_Вогеля")
        # добавим лист с суммой стоимости (как отдельный маленький лист)
        ...
        # 5) при наличии оптимизации добавляем лист
